...
async function main() {
  ...
  for (const pending of pendingBatch) {
    ...
    // fullTime puede incluir los penales sumados cuando hubo tanda (bug observado en la API)
    // Si hay penalties y regularTime/extraTime disponibles, el marcador real es regularTime+extraTime
    const hasPenalties = fm.score.penalties && fm.score.penalties.home !== null && fm.score.penalties.home !== undefined;
    let homeGoals, awayGoals;
    if (hasPenalties && fm.score.regularTime) {
      const reg = fm.score.regularTime;
      const ext = fm.score.extraTime || { home: 0, away: 0 };
      homeGoals = reg.home + (ext.home || 0);
      awayGoals = reg.away + (ext.away || 0);
    } else {
      homeGoals = fm.score.fullTime.home;
      awayGoals = fm.score.fullTime.away;
    }
    console.log(`  📊 Score API: ${found.home} ${homeGoals}-${awayGoals} ${found.away} (status=${fm.status}, fullTime=${fm.score.fullTime?.home}-${fm.score.fullTime?.away}, regularTime=${fm.score.regularTime?.home}-${fm.score.regularTime?.away}, extraTime=${fm.score.extraTime?.home}-${fm.score.extraTime?.away}, penalties=${fm.score.penalties?.home}-${fm.score.penalties?.away})`);
